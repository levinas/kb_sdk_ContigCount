package ContigCount::ContigCountClient;
...
sub new
{
    ...
    my $self = {
	client => ContigCount::ContigCountClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
$return is a ContigCount.CountContigsResults
...
# Authentication: required
	...
	method => "ContigCount.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for ContigCount::ContigCountClient\n";
    }
    if ($sMajor == 0) {
        warn "ContigCount::ContigCountClient version is $svr_version. API subject to change.\n";
    }
}
...
package ContigCount::ContigCountClient::RpcClient;
